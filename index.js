...
const obradiNepostojanjeSesije = (request, response) => {
  if (!request.session.korisničkoIme) {
    ...
    response.send(
      JSON.stringify({
        poruka: "Korisnik nije prijavljen na korisnički račun!",
      })
    );
    return true;
  }
...
const sumirajNutritivnuVrijednost = (namirnice, nazivNutritivneVrijednosti) => {
  return namirnice
    .map((element) => element[nazivNutritivneVrijednosti])
    .reduce((vrijednost, suma) => (suma = suma + vrijednost), 0);
};
...
const sumirajDuplikate = (namirnice) => {
  ...
  for (let i = 0; i < namirnice.length; i++)
    if (
      jedinstveniElementi.filter((element) => element.id == namirnice[i].id)
        .length == 0
    )
      jedinstveniElementi.push({ ...namirnice[i] });
  jedinstveniElementi.forEach((jedinstveniElement) => {
    duplikatiJedinstvenogElementa = namirnice.filter(
      (element) => element.id == jedinstveniElement.id
    );
    const sumiraneVrijednostiJedinstvenogElementa =
      sumirajNutritivneVrijednosti(duplikatiJedinstvenogElementa);
    jedinstveniElement.energija =
      sumiraneVrijednostiJedinstvenogElementa.energija;
    jedinstveniElement.proteini =
      sumiraneVrijednostiJedinstvenogElementa.proteini;
    jedinstveniElement.masti = sumiraneVrijednostiJedinstvenogElementa.masti;
    jedinstveniElement.ugljikohidrati =
      sumiraneVrijednostiJedinstvenogElementa.ugljikohidrati;
    jedinstveniElement.vitaminA =
      sumiraneVrijednostiJedinstvenogElementa.vitaminA;
    jedinstveniElement.vitaminE =
      sumiraneVrijednostiJedinstvenogElementa.vitaminE;
    jedinstveniElement.vitaminD =
      sumiraneVrijednostiJedinstvenogElementa.vitaminD;
    jedinstveniElement.vitaminC =
      sumiraneVrijednostiJedinstvenogElementa.vitaminC;
    jedinstveniElement.željezo =
      sumiraneVrijednostiJedinstvenogElementa.željezo;
    jedinstveniElement.magnezij =
      sumiraneVrijednostiJedinstvenogElementa.magnezij;
    ...
    jedinstveniElement.gramaža = duplikatiJedinstvenogElementa
      .map((element) => element.gramaža)
      .reduce((vrijednost, suma) => (suma = suma + vrijednost), 0);
...
application.post("/registracija", (request, response) => {
  if (obradiPostojanjeSesije(request, response)) return;
  bazaPodataka
    .kreirajKorisničkiRačun(request.body)
    .then((validacijaPodataka) => {
      response.setHeader("Content-Type", "application/json");
      response.status(
        validacijaPodataka.email &&
          validacijaPodataka.korisničkoIme &&
          validacijaPodataka.lozinka
          ? 200
          : 400
      );
...
application.post("/prijava", (request, response) => {
  if (obradiPostojanjeSesije(request, response)) return;
  bazaPodataka
    .izvršiPrijavuNaKorisničkiRačun(request.body)
    .then((validacijaPodataka) => {
      const validno =
        validacijaPodataka.korisničkoIme && validacijaPodataka.lozinka;
      if (validno) request.session.korisničkoIme = request.body.korisničkoIme;
      response.setHeader("Content-Type", "application/json");
...
application.post("/kreiranjeSigurnosnogTokena", (request, response) => {
  if (obradiPostojanjeSesije(request, response)) return;
  response.setHeader("Content-Type", "application/json");
  bazaPodataka.kreirajSigurnosniToken(request.body.email).then((rezultat) => {
    if (rezultat.email)
      komunikacija
        .pošaljiEmail(
          rezultat.email,
          "NuThyro | Promjena Lozinka",
          kreirajSadržajEmaila(rezultat.token)
        )
        .then(() => {
          response.status(200);
...
application.post("/potvrdaSigurnosnogKoda", (request, response) => {
  if (obradiPostojanjeSesije(request, response)) return;
  bazaPodataka
    .provjeriSigurnosniToken(request.body.email, request.body.sigurnosniKod)
    .then((rezultat) => {
      response.setHeader("Content-Type", "application/json");
      response.status(rezultat.email ? 200 : 400);
      response.send(
        JSON.stringify({
          sigurnosniKod: rezultat.token ? request.body.sigurnosniKod : null,
        })
      );
    });
});
...
application.post("/promjenaLozinke", (request, response) => {
  if (obradiPostojanjeSesije(request, response)) return;
  bazaPodataka
    .promijeniLozinkuZaKorisničkiRačun(
      request.body.email,
      request.body.sigurnosniKod,
      request.body.novaLozinka
    )
    .then((rezultat) => {
      const validacija =
        rezultat.email && rezultat.token && rezultat.novaLozinka;
      response.setHeader("Content-Type", "application/json");
...
application.get("/tipoviDnevneAktivnosti", (request, response) => {
  if (obradiNepostojanjeSesije(request, response)) return;
  bazaPodataka.dajSveTipoveAktivnosti().then((tipoviAktivnosti) => {
    response.setHeader("Content-Type", "application/json");
    response.status(200);
...
application.post("/dodavanjeEnergetskihVrijednosti", (request, response) => {
  if (obradiNepostojanjeSesije(request, response)) return;
  bazaPodataka
    .dodajEnergetskeVrijednostiZaKorisnika(
      request.session.korisničkoIme,
      request.body.bmr,
      request.body.tdee,
      request.body.bmi
    )
    .then(() => {
      ...
      response.send(
        JSON.stringify({
          poruka: "Uspješno dodane energetske vrijednosti za korisnika!",
        })
      );
    });
});
...
application.get("/energetskeVrijednostiZaKorisnika", (request, response) => {
  ...
  response.setHeader("Content-Type", "application/json");
  bazaPodataka
    .dajEnergetskeVrijednostiZaKorisnika(request.session.korisničkoIme)
    .then((rezultat) => {
      if (!rezultat.bmr) {
        response.status(404);
        response.send(
          JSON.stringify({ poruka: "Korisnik još uvijek nije popunio formu!" })
        );
        return;
      }
      response.status(200);
...
application.get("/dostupneNamirnice", (request, response) => {
  if (obradiNepostojanjeSesije(request, response)) return;
  response.setHeader("Content-Type", "application/json");
  bazaPodataka.dajDostupneNamirnice().then((rezultat) => {
    rezultat.forEach((namirnica) => {
      delete namirnica.dataValues.ikona;
    }); // Ne vraća se zapis ikone (to radi odvojena ruta)!
    response.status(200);
...
application.get(/\/ikonaNamirnice\/[1-9]\d*/, (request, response) => {
  ...
  if (isNaN(id)) {
    ...
    response.status(400);
    response.send(
      JSON.stringify({ poruka: "Nevalidan format ID-a namirnice!" })
    );
  }
  bazaPodataka.dajIkonuNamirnice(id).then((rezultat) => {
    if (rezultat === null) {
      response.setHeader("Content-Type", "application/json");
      response.status(404);
      response.send(
        JSON.stringify({
          poruka: "Namirnica sa datom ID vrijednosti ne postoji!",
        })
      );
      return;
    }
...
application.post("/dodajNamirnicuNaSpisak", (request, response) => {
  if (obradiNepostojanjeSesije(request, response)) return;
  response.setHeader("Content-Type", "application/json");
  bazaPodataka.postojiLiNamirnica(request.body.namirnica).then((rezultat) => {
    ...
    else request.session.spisakNamirnica.push(request.body.namirnica);
    response.status(200);
...
application.post("/ukloniNamirnicuSaSpiska", (request, response) => {
  ...
  response.setHeader("Content-Type", "application/json");
  if (
    request.session.spisakNamirnica === undefined ||
    request.session.spisakNamirnica.filter(
      (element) =>
        element.id == request.body.id && element.gramaža == request.body.gramaža
    ).length == 0
  ) {
    response.status(404);
    response.send(
      JSON.stringify({ poruka: "Namirnica nije ni bila na spisku!" })
    );
    return;
  }
  const spisak = request.session.spisakNamirnica;
  request.session.spisakNamirnica = [
    ...spisak.slice(
      0,
      spisak.findIndex(
        (element) =>
          element.id == request.body.id &&
          element.gramaža == request.body.gramaža
      )
    ),
    ...spisak.slice(
      spisak.findIndex(
...
application.get("/dajSumarneNutritivneVrijednosti", (request, response) => {
  ...
  response.send(
    JSON.stringify({
      ...
      pojedinačneVrijednosti:
        namirnice === undefined ? [] : sumirajDuplikate(namirnice),
      sumarneVrijednosti: sumirajNutritivneVrijednosti(
        namirnice === undefined ? [] : namirnice
      ),
    })
  );
});
...
application.get(
  "/googlePrijava",
  obradiPostojanjeSesijeGoogleOAuth2,
...
application.get(
  "/googlePrijavaPovratak",
  obradiPostojanjeSesijeGoogleOAuth2,
  (request, response) => {
    passport.authenticate("google", (greška, korisnik, info) => {
      if (!korisnik) response.redirect("/html/prijava.html");
      else {
...
application.listen(3000, (greška) => {
  if (greška) console.log("Greška prilikom pokretanja servera!");
  else console.log("Server osluškuje na portu 3000...");
});
